...
{
	// N3485 8.2/7: a type-name nested in parentheses in a parameter clause is
	// a simple-type-specifier rather than a declarator-id, so `int(value_type)`
	// declares a parameter of function type and the parentheses hold that
	// parameter clause.  The node keeps the name the named form gives it.
	if(At(posttoken::OP_LPAREN) && At(kIdentifierToken, 1) && At(posttoken::OP_RPAREN, 2) &&
	   IsTypeName(Spelling(1)))
	{
		const int node = Tag("declarator");
		Add(node, ParameterClause());
		return node;
	}
	// `( ... )` is a parameter clause with a pack, never a parenthesized
	// declarator, because `...` is not a declarator-id.
	if(At(posttoken::OP_LPAREN) && At(posttoken::OP_DOTS, 1))
	{
...
// Counts the `ptr-operator` nodes a declarator carries, which is what decides
// how an abstract declarator in a parameter prints.  A parameter clause holds
// another declarator's tree, so a pointer operator inside one is not this
// declarator's: `int(C*)` keeps the abstract reading and `int(*)` does not.
