#!/usr/bin/perl
#
# Compiler benchmark for the pa6 scope/type stage.
#
# Generates one fixed, reproducible declaration-heavy translation unit, checks
# that the student compiler and the reference wrapper agree on the scope dump
# byte for byte, and reports paired ABBA wall-time and peak-RSS blocks for both.
# A second A/A arm measures the reference against itself, so the noise floor of
# the schedule is visible beside the effect it is used to judge.
#
# The corpus is the shape this stage is actually paid for: declarations in
# nested namespaces, class and enumeration definitions with member scopes,
# typedefs and alias declarations, array declarators whose bounds are constant
# expressions, and qualified and unqualified lookups into all of them.  The
# semantic work tracks the declarations, the scopes they open and the lookup
# candidates a name walks past, so the corpus is sized by group count and the
# dump's line count is reported as the size counter the work produces.
#
...
# is still printed so an excursion larger than the measured effect stays
# visible.
#
# `cppgm++ --emit-types` has no executable output, so only compiler latency,
# peak RSS and the size of the dump are reported; there is no generated-program
# runtime or text size at this stage, and no telemetry surface is invented to
# report one.
#
# This is a personal benchmark: it is not part of the course contract and is not
# discovered by `make test`.
#
#   perl student.tests/types_benchmark.pl [groups] [blocks]
...
my $groups = shift @ARGV // 6000;
my $blocks = shift @ARGV // 5;
my $outdir = '/tmp/pa6_types_benchmark';
my $corpus = "$outdir/types_bench.t";
my $tsv = "$outdir/types_benchmark.tsv";

my $mine = "$root/dev/cppgm++";
my $ref = "$root/pa6/cppgm++-ref";
...
namespace inner {
  ...
  namespace deeper {
    using alias_t = node;
    typedef alias_t* link_t;
  }
}

namespace reopened { typedef char small_t; }

}

namespace bench { namespace reopened { small_t marker; } }

HEAD
	for my $i (1 .. $count)
	{
		my $n = 1 + ($i % 5);
		print $fh "bench::point p_$i;\n";
		print $fh "bench::point::coord c_$i;\n";
		print $fh "bench::color col_$i;\n";
		print $fh "bench::inner::node n_$i;\n";
		print $fh "bench::inner::deeper::link_t l_$i;\n";
		print $fh "int a_${i}[bench::green];\n";
		print $fh "int b_${i}[bench::point::area];\n";
		print $fh "bench::index_t i_$i;\n";
		print $fh "bench::const_index_t k_$i = $n;\n";
		print $fh "void f_$i(bench::point* p, bench::inner::node& r) { (void)p; (void)r; }\n";
	}
	close($fh);
	return -s $path;
}
...
sub measure_latency
{
	...
	my $start = time();
	system("$tool --emit-types -o $output $input > /dev/null 2>&1");
...
{
	my ($tool, $input, $output) = @_;
	my $timing = `{ /usr/bin/time -f '%M' $tool --emit-types -o $output $input > /dev/null; } 2>&1`;
	chomp($timing);
	my ($rss) = $timing =~ /(\d+)\s*\z/;
...
system("$mine --emit-types -o $outdir/mine.txt $corpus > /dev/null 2>&1") == 0
	or die "student compiler failed on the corpus\n";
system("$ref --emit-types -o $outdir/ref.txt $corpus > /dev/null 2>&1") == 0
...
close($dump);
printf "dump: %d bytes, %d lines (one per declaration or scope), byte-identical\n",
